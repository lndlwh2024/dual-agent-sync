...
Each line of .ai-sync/collab/ledger.jsonl is one JSON object.
...
`json
{
  ...
  "feature_scope": "Optional. The functional scope of this change.",
  "git": {
    "branch": "master",
    "base_commit": "abc123",
    "head_commit": "def456"
    ...
    "files_changed": [
      {
        "file": "src/file.ts",
        "lines": ["10-20", "45-50"],
        "purpose": "Optional. Why this file was modified."
      }
    ],
  ...
  "graph_impact": {
    ...
    "nodes_added": [],
    "nodes_removed": [],
    "edges_added": [{"from": "A", "to": "B", "type": "imports"}],
    "edges_removed": []
  },
  "context": {
...
- In an append-only ledger, line N (1-indexed) corresponds to version {N:04d} (where the init event v0000 is not stored, so line 1 = v0001, line 2 = v0002, etc.). This stable mapping enables incremental reads by line number.
...
Each AI IDE maintains one cursor file at .ai-sync/collab/cursors/<ai-ide-id>.json. The file tracks read positions per conversation window (session).
...
|-------|------|----------|-------------|
| i_ide_id | string | Yes | The AI IDE identifier |
