...
- **v1.5 (Latest)**：
  - 🔬 **锁协议语义精确化（由 Codex 并发模拟验证推动）**：
    - **Step 1 更名并强化语义**：从“先检查再写入”（check-then-write，存在 TOCTOU 竞争窗口）升级为 **CreateNew / O_EXCL 独占写入语义**——不存在时写入有效，文件已存在时禁止覆盖（视同独占失败），明确消除名称与实现不符的语义误导。
    - **新增 Design Note 说明**：在协议头部增加设计注释，解释 AI 工具层无法使用 OS 原语的原因，以及 Step 1 + Step 2 联合近似原子性的工作原理。
    - **Step 2 明确为「真正的序列化仲裁点」**：补充注释说明：一旦 Step 2 确认归属，其他合规 Agent 不会再覆盖当前锁，Step 2 是协议的确定性序列化边界。
